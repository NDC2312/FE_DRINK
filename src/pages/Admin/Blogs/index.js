...
function BlogsInAdmin() {
    ...
    useEffect(() => {
        ...
        const fetch = async () => {
            const res = await blogsService.getBlogs(params);
            setData(res.newBlogs);
            setTotalPages(Math.ceil(res.countBlogs / 4));
        };
        fetch();
    ...
    const handleChangMulti = async () => {
        ...
        if (idChecked.length > 0) {
            ...
            const res = await blogsService.changeMultiBlogs(oke);
            console.log(res);
            let updateData = [...data];
            if (type === 'status') {
    ...
    const handleBtnDelete = async (id) => {
        if (window.confirm('Bạn chắc chắn muốn xóa sản phẩm này chứ.')) {
            const res = await blogsService.deleteBlogs(id);
            ...
            <div className={cx('container')}>
                ...
                <div className={cx('act-product')}>
                    ...
                    <div className={cx('form-input')}>
                        ...
                        <Button to={config.routes.adminAddBlogs} btnAddNew>
                            + Thêm mới
                        </Button>
                    </div>
                </div>
