...
function AdminAuth() {
    ...
    const handleBtnDelete = async (id) => {
        if (window.confirm('Bạn chắc chắn muốn xóa sản phẩm này chứ.')) {
            const res = await AuthService.deleteAuth(id);
            console.log(res);
        }
    };
    ...
    useEffect(() => {
        const fetch = async () => {
            const res = await AuthService.getAuth();
            setData(res);
        };
        fetch();
    }, []);
                        ...
                        {data &&
                            data.map((data, index) => (
                                <tr key={data._id}>
                                    ...
                                    <td>
                                        <Link
                                            to={config.routes.adminDetailAuth.replace(':id', data._id)}
                                            className={`${cx('btn-detail')} ${styles.btn}`}
                                        >
                                            Chi tiết
                                        </Link>
                                        <Link
                                            to={config.routes.adminEditAuth.replace(':id', data._id)}
                                            state={{ account: data }}
