...
export const getProducts = async (params) => {
    try {
        const res = await httpRequest.get('ingredient', { params });
        return res.data;
    } catch (error) {
        console.log(error);
    }
...
export const getSupplier = async () => {
    try {
        const res = await httpRequest.get('ingredient/supplier');
        return res.data;
    } catch (error) {
        console.log(error);
    }
...
export const addProduct = async (data) => {
    try {
        const res = await httpRequest.post('ingredient/create', data);
        console.log(res);
        if (res.data.code === 200) {
            success(res.data.message);
        } else {
